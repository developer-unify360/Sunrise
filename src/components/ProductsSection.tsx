    ...
    <section id="products" className="bg-gray-50 py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          <div>
            <div className="inline-flex items-center gap-2 mb-4">
              ...
              <span className="text-orange-500 text-xs font-bold uppercase tracking-[0.3em]">Product Range</span>
            </div>
          ...
          <Link
            to="/products"
            className="hidden md:inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs uppercase tracking-widest px-6 py-3 transition-colors shadow-md"
          >
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {mainCategories.map((cat) => (
            <Link
              key={cat.id}
              to="/products"
              className="group bg-white border border-gray-200 hover:border-orange-500 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 block"
            >
              ...
              <div className="relative overflow-hidden h-52">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <h3 className="text-white font-black text-sm uppercase leading-tight tracking-wide group-hover:text-orange-300 transition-colors">
                    {cat.title}
                  </h3>
                </div>
              </div>
              ...
              <div className="p-4">
                <ul className="space-y-1">
                  {cat.items.slice(0, 4).map((item) => (
                    <li key={item} className="text-gray-500 text-xs flex items-start gap-1.5">
                      <span className="w-1 h-1 bg-orange-500 rounded-full flex-shrink-0 mt-[5px]" />
                      {item}
                    </li>
                  ))}
                  {cat.items.length > 4 && (
                    <li className="text-orange-500 text-xs font-bold mt-0.5">
                ...
                <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between">
                  ...
                  <ArrowRight size={13} className="text-orange-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
              </div>
            </Link>
          ))}
        ...
        <div className="text-center mt-10 md:hidden">
          <Link
            to="/products"
            className="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs uppercase tracking-widest px-8 py-4 transition-colors"
          >
            View Full Catalogue <ArrowRight size={14} />
          </Link>
        </div>
